...
void calc_form_factor(const int a_div_num, const int b_div_num, const int mc_sample) 
{
	...
	for (int i = 0; i < int(n); i++) 
	{
		recs[i].divide_patchs(a_div_num, b_div_num); // Divide appropriately for the time being
	...
	for (int i = 0; i < int(n); i++) 
	{
		...
		for (int ia = 0; ia < recs[i].a_num; ia++) 
		{
			...
			{
				// Patch (ia, ib) on face (i)
				// form_factor[patch_i * patch_num + patch_j] = F_ij
				const Vec normal_i = recs[i].normal;
				const double Ai = Cross(recs[i].a / recs[i].a_num, recs[i].b / recs[i].b_num).Length();	// Area of ​​patch (ia, ib)
...
{
	// Solve simultaneous linear equations (radiosity equations) with the Gauss-Seidel method
	const double n = sizeof(recs) / sizeof(Rectangle);
	int patch_i = 0;
	for (int i = 0; i < int(n); i++) 
	{
...
Color radiance(const Ray& ray, const int depth, bool interpolation = true) 
{
	...
	// All geometry that makes up the scene is assumed to be a perfect diffusion surface
	switch (DIFFUSE) 
	{
		case DIFFUSE: {
			...
			// PI times because it is a perfect diffusion surface
...
int main(int argc, char** argv) 
{
	...
	// Form factor calculation
	// Divide into 16x16, Monte Carlo integration using (3x3) ^ 2 sample for form factor calculation
	calc_form_factor(16, 16, 3);

	// Solving the radiosity equation with the Gauss-Seidel method
	// 32 iterations
	for (int i = 0; i < 32; i++) 
	{
		std::cout << i << " ";
